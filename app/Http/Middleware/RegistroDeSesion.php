...
class RegistroDeSesion
{
    ...
    public function handle($request, Closure $next)
    {
        if (!$request->session()->has('sesion_id'))
        {
            ...
            $Sesion->ip_adress = substr($ip_adress, 1, 20);
            $Sesion->tipo_de_dispositivo = $tipo_de_dispositivo;
            $Sesion->user_agent = substr($user_agent, 1, 50);
            $Sesion->lenguaje = substr($lenguaje, 1, 12);
            $Sesion->es_un_bot = $es_un_bot;
            $Sesion->pagina_de_origen = substr(URL::previous(), 1, 80);
            $Sesion->pagina_de_ingreso = substr(Request::fullUrl(), 1, 80);

            $Sesion->browser_name = substr(Browser::browserName(), 1, 35);
            //$Sesion->browser_name = 'ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss';
            $Sesion->browser_version = substr(Browser::browserVersion(), 1, 15);
            $Sesion->browser_family = substr(Browser::browserFamily(), 1, 25);
            $Sesion->browser_engine = substr(Browser::browserEngine(), 1, 10);
            $Sesion->platform_name = substr(Browser::platformName(), 1, 25);
            $Sesion->platform_family = substr(Browser::platformFamily(), 1, 15);
            $Sesion->platform_version = substr(Browser::platformVersion(), 1, 10);
            $Sesion->device_family = substr(Browser::deviceFamily(), 1, 40);
            $Sesion->device_model = substr(Browser::deviceModel(), 1, 40);
            $Sesion->mobile_grade = substr(Browser::mobileGrade(), 1, 5);
